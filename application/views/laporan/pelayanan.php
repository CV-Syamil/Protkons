	...
	$(document).ready(()=>{
		var tbl = $('#datatable').DataTable({
	        ...
	        columns: [
	        	...
	        	{data: 'jml_berkas', className:'text-right'},
	        	{data: 'total_biaya', className:'text-right', searchable: false, orderable: false},
	        ]
		});
        ...
        $('#cari_date').daterangepicker({
			...
			start_date = start; end_date = end; tbl.ajax.reload();
			setValDate();
		});
		$('#btn_print').on('click',()=>{
			window.open('<?=site_url('laporan/pelayanan/cetak')?>?start_date='+start_date.format('YYYY-MM-DD')+'&end_date='+end_date.format('YYYY-MM-DD'));
		});
		setValDate();
	});
</script>