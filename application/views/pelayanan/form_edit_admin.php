...
<div class="card card-solid">
	...
	<div class="card-body">
		<?=form_open_multipart('pelayanan/simpan-data2',['id'=>'form_tr_pelayanan'],['ref'=>@$data->id,'pl_ref'=>$pl->pelayanan_id,'pelapor'=>$pelapor->id])?>
		<div class="table-responsive">
			<table class="table table-striped table-borderless">
				<tr>
					<td colspan="3">
						<span class="h5">DATA PELAYANAN</span>
					</td>
				</tr>
				<tr>
					<td width="175">Pelayanan</td>
					<td width="5">:</td>
					<td><?=$pl->pelayanan?></td>
				</tr>
				<tr>
					...
					<td>:</td>
					<td>
						<select name="user_hs" class="form-control">
							<option value="">-</option>
							<?php foreach ($user_hs as $v) {
								$s = ($v->user_id==$data->user_hs)?'selected':'';
								echo "<option value=\"$v->user_id\" $s>$v->nama</option>";
							} ?>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="status" class="form-control">
							<?php foreach (status_layanan() as $key => $v) {
								$s = ($key==$data->status)?'selected':'';
								echo ($key<=$data->status)?"<option value=\"$key\" $s>$v</option>":'';
							} ?>
						</select>
					</td>
				</tr>
		...
		<div class="table-responsive">
			<table class="table table-striped table-hover" style="text-transform:uppercase;">
				<tr>
					<td colspan="3">
						<span class="h5">Data Pelapor</span>
						<button class="btn btn-sm btn-warning" onclick="cari_identitas('pelapor_xxx');" style="float:right" type="button"><i class="fa fa-edit"></i> Ubah Pelapor</button>
					</td>
				</tr>
				<tr>
					<td width="175">No. Identitas</td>
					<td width="5">:</td>
					<td id="no_identitas_pelapor_xxx"><?=$pelapor->no_identitas?></td>
				</tr>
				<tr>
					<td>Nama Lengkap</td>
					<td>:</td>
					<td id="nama_pelapor_xxx"><?=$pelapor->nama?></td>
				</tr>
				<tr>
					<td>Jenis Kelamin</td>
					<td>:</td>
					<td id="jk_pelapor_xxx"><?=$pelapor->jk?></td>
				</tr>
				<tr>
					<td>Tempat, Tanggal Lahir</td>
					<td>:</td>
					<td><span id="tempat_lahir_pelapor_xxx"><?=$pelapor->tempat_lahir?></span>, <span id="tgl_lahir_pelapor_xxx"><?=tanggal_indo($pelapor->tgl_lahir)?></span></td>
				</tr>
				<tr>
					<td>Umur</td>
					<td>:</td>
					<td id="umur_pelapor_xxx"><?=get_umur($pelapor->tgl_lahir)?> th</td>
				</tr>
				<tr>
					<td>Agama</td>
					<td>:</td>
					<td id="agama_pelapor_xxx"><?=$pelapor->agama?></td>
				</tr>
				<tr>
					<td>Pekerjaan</td>
					<td>:</td>
					<td id="pekerjaan_pelapor_xxx"><?=$pelapor->pekerjaan?></td>
				</tr>
				<tr>
					<td>Kewarganegaraan</td>
					<td>:</td>
					<td id="kewarganegaraan_pelapor_xxx"><?=$pelapor->kewarganegaraan?></td>
				</tr>
				<tr>
					<td>Provinsi</td>
					<td>:</td>
					<td id="provinsi_pelapor_xxx"><?=strtoupper($pelapor->provinsi)?></td>
				</tr>
				<tr>
					<td>Kota / Kabupaten</td>
					<td>:</td>
					<td id="kota_pelapor_xxx"><?=strtoupper($pelapor->kota)?></td>
				</tr>
				<tr>
					<td>Kecamatan</td>
					<td>:</td>
					<td id="kecamatan_pelapor_xxx"><?=strtoupper($pelapor->kecamatan)?></td>
				</tr>
				<tr>
					<td>Alamat Lengkap</td>
					<td>:</td>
					<td id="alamat_idn_pelapor_xxx"><?=$pelapor->alamat_idn?></td>
				</tr>
				<tr><td colspan="3">&nbsp;</td></tr>
				<tr>
					<td>Negeri</td>
					<td>:</td>
					<td id="negeri_pelapor_xxx"><?=strtoupper($pelapor->negeri)?></td>
				</tr>
				<tr>
					<td>Daerah</td>
					<td>:</td>
					<td id="daerah_pelapor_xxx"><?=strtoupper($pelapor->daerah)?></td>
				</tr>
				<tr>
					<td>Distrik</td>
					<td>:</td>
					<td id="distrik_pelapor_xxx"><?=strtoupper($pelapor->distrik)?></td>
				</tr>
				<tr>
					<td>Alamat Lengkap</td>
					<td>:</td>
					<td id="alamat_mys_pelapor_xxx"><?=$pelapor->alamat_mys?></td>
				</tr>
			</table>
		</div>
		<?php $modal_identitas=TRUE; $jcombo=[]; $date_input=[]; $time_input=[]; foreach($fields as $field){ 
		...
		var pilih_identitas = (href)=>{
			...
			$.ajax({
				...
				success: (r)=>{
					...
					if(r.data){
						r.data.forEach((e,i)=>{
							if(tipe=='pelapor_xxx'&&e[0]=='id'){
								$('input[name="pelapor"]').val(e[1]);
							}
							if($('#'+e[0]+'_'+tipe).is('input')){
								$('#'+e[0]+'_'+tipe).val((e[1])?e[1]:'-');
							}else{
								$('#'+e[0]+'_'+tipe).html((e[1])?e[1]:'-');
							}
						});
					}
				}
